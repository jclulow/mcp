...
#define	MAXMAPSIZE	(1024*1024*16)	/* map at most 16MB */
    ...
    volatile uint64_t *progress)
{
	...
	if (mapsize != 0) {
		...
		for (;;) {
			...
			if (nbytes >= 0 && progress != NULL) {
				atomic_add_64(progress, nbytes);
			}
...
int
builder_copy_file(const char *src, const char *dst, volatile uint64_t *progress)
{
	int error = 0;
	int fsrc = -1, fdst = -1;
	struct stat stsrc;
	...
	if (builder_writefile(fsrc, fdst, &stsrc, progress) != 0) {
		error = errno;
		goto out;
	}
