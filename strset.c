...
struct strset_cursor {
	strset_t *ssc_set;
	strset_node_t *ssc_pos;
};

int
strset_cursor(strset_t *ss, strset_cursor_t **sscp)
{
	strset_cursor_t *ssc;

	if ((ssc = calloc(1, sizeof (*ssc))) == NULL) {
		return (-1);
	}

	ssc->ssc_set = ss;
	ssc->ssc_pos = avl_first(&ss->ss_strings);
...
void
strset_cursor_free(strset_cursor_t *ssc)
{
	free(ssc);
}

const char *
strset_cursor_peek(strset_cursor_t *ssc)
{
	if (ssc->ssc_pos == NULL) {
		return (NULL);
	}

	return (ssc->ssc_pos->ssn_string);
}

void
strset_cursor_next(strset_cursor_t *ssc)
{
	if (ssc->ssc_pos == NULL) {
		return;
	}

	ssc->ssc_pos = AVL_NEXT(&ssc->ssc_set->ss_strings, ssc->ssc_pos);
}
